  ...
  return (
    <div className="h-screen flex flex-col text-center p-5 bg-gray-800">
      ...
      <div className="w-[60%] ml-auto mr-auto mt-5 flex sm:flex-col sm:flex-col md:flex-row bg-gray-800 rounded-[30px] p-5 align-center justify-evenly bg-gray-900 text-white border border-gray-700 transition duration-200">
        { grafico === 0 ? <GraficoVendas produto={produto} /> : <GraficoPizza /> }
        <div className="md:w-fit sm:w-[100%] flex flex-col">
          <button className="flex items-center gap-2 mt-5 sm:ml-5 px-6 py-3 rounded-full font-semibold text-black transition duration-500 ease-in-out cursor-pointer bg-gray-700 hover:bg-gray-100 shadow-lg" onClick={() => { grafico === 0 ? setGrafico(1) : setGrafico(0)}}>Alterar visualização</button>
